...
export default function JobDetailPage() {
  ...
  return (
    <div className="space-y-6">
              ...
              <div className="relative">
                ...
                {stageData.credit_warning && (
                  <div className="mt-4 flex items-start gap-2.5 rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-amber-400">Low credits</p>
                      <p className="text-xs text-amber-400/70 mt-0.5">
                        {stageData.profiles_needed
                          ? `Found ${formatNumber(stageData.profiles_needed as number)} unique profiles but you only have ${formatNumber(stageData.credit_balance as number)} credits. Only ${formatNumber(stageData.profiles_affordable as number)} profiles will be enriched.`
                          : `${formatNumber(stageData.profiles_skipped_no_credits as number)} profiles skipped due to insufficient credits.`
                        }
                      </p>
                    </div>
                  </div>
      ...
      {/* Export success — floating modal with audience hint */}
      {exportNotice && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setExportNotice(null)} />
          <div className="relative w-full max-w-md rounded-2xl border border-white/10 bg-[#0d1117] shadow-2xl shadow-black/50 p-6 space-y-4 animate-in fade-in zoom-in-95 duration-200">
            {/* Close button */}
            <button
            ...
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-full bg-emerald-500/15">
                <svg className="h-5 w-5 text-emerald-400" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" /></svg>
              </div>
              <div>
                <h3 className="text-base font-semibold text-white">Export Downloaded</h3>
                <p className="text-xs text-white/50 mt-0.5">{exportNotice}</p>
              </div>
            </div>
