                          ...
                          {(() => {
                            ...
                            return <>
                              <input placeholder={field.placeholder}
                                value={isArray ? `[${val.length} variations] ${val[0] || ""}...` : (val || "")}
                                onChange={(e) => setSbParams(prev => ({
                                  ...prev,
                                  [action.key]: { ...(prev[action.key] || {}), [field.name]: e.target.value }
                                }))}
                                className={`w-full bg-black/20 border rounded px-2 py-1.5 text-xs text-white placeholder-white/20 focus:outline-none ${isArray ? "border-blue-500/30" : "border-white/10"}`} />
                              <p className="text-[10px] text-white/15 mt-0.5">{isArray ? `${val.length} unique values generated by AI` : field.hint}</p>
                            </>;
                          })()}
                        </div>
                      ))}
