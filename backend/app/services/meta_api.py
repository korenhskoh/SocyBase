...
class MetaAPIService:
    ...
    def _raise_meta_error(resp: httpx.Response) -> None:
        """Raise an HTTPStatusError with the Facebook error message, stripping the access token.

        Facebook returns errors as: {"error": {"message": "...", "type": "...", "code": N}}
        The default httpx error includes the full URL with the access token, which must not
        be exposed to end users.
        """
        if resp.is_success:
            return
        # Try to extract the Facebook-specific error message
        fb_message = ""
        try:
            body = resp.json()
            err = body.get("error", {})
            fb_message = err.get("message") or err.get("error_user_msg") or ""
            fb_type = err.get("type", "")
            fb_code = err.get("code", "")
            if fb_message:
                fb_message = f"Meta API error ({fb_type} {fb_code}): {fb_message}"
        except Exception:
            pass
        if not fb_message:
            fb_message = f"Meta API returned HTTP {resp.status_code}"
        raise httpx.HTTPStatusError(
            fb_message,
            request=httpx.Request(resp.request.method, str(resp.request.url).split("?")[0]),
            response=resp,
        )
